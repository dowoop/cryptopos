...
| `_Unresolved`, `_Unresolved.__init__` | carry *the provider did not answer* as a thing distinct from *the provider said no* — the first is retried, the second is a verdict, and collapsing them cost an honest payment its sale on one HTTP 503 | `test_ootle.WhatTheProviderDidNotSay` |
| `_observed_transfer`, `_observed_transfers` | the **one** implementation of *is this transaction money?* — a deposit is credited only when the indexer's own summary says the transaction committed, and an uncommitted one is reported unconfirmed rather than dropped so an operator sees it. Shared by both the account path and the component path, because D35 is what the same rule in two places costs | `test_ootle.OotleRailTest`, `PaymentComponentBinding` |
| `OotleEsmeralda._replay` | one bounded page of an event stream, resumed from a cursor — and the one place that decides whether an empty read is the end of a history or a failure | `test_ootle.OotleRailTest`, `PaymentComponentBoundaries` |
| `OotleEsmeralda._resolve_outstanding` | re-ask the provider about transactions a previous poll could not establish, so a transient read failure costs a retry rather than the sale — without it the fix for a false refusal is only a sale that expires quietly instead | `test_ootle.DoubtIsNotPermanent` |
| `OotleEsmeralda._drain` | make a baseline cursor mean what it claims — replay to the true end of the history rather than to the first place a bounded read stopped, and refuse rather than guess when it will not end | `test_ootle.OotleRailTest`, `PaymentComponentBoundaries` |
| `OotleEsmeralda._payment_component`, `OotleEsmeralda._payment_events`, `OotleEsmeralda._sale_reference` | the per-sale binding's configuration, its event stream and the reference a payer must name; a deployment pointed at a payment component observes that component rather than a shared vault | `test_ootle.PaymentComponentBinding`, `PaymentComponentBoundaries` |
| `OotleEsmeralda._referenced_payments` | credit only payments that **name this sale**, and refuse — never silently skip — an event this build cannot read, because dropping one would under-credit a customer who really paid | `test_ootle.PaymentComponentBinding`, `PaymentComponentBoundaries` |
