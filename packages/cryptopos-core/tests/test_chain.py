...
def reading(transport):
	...
	return mock.patch("cryptopos_core.chain._urlopen", transport)
...
class Construction(unittest.TestCase):
	...
	def test_malformed_user_agent_falls_back_to_text(self):
		...
		self.assertTrue(reader.user_agent.startswith("cryptopos-core/"))

	def test_identifies_itself(self):
		# An indexer operator gets to see who is reading.
		from cryptopos_core import __version__

		self.assertEqual(OotleReader().user_agent, "cryptopos-core/" + __version__)
